    ...
    <div class="control-section" v-if="showStocksChart">
      <div align="center">
        <ejs-accumulationchart
          ...
          align="center"
          id="chartcontainer2"
          :title="'% activos de tu cartera'"
          :legendSettings="legendSettings"
          :tooltip="tooltip"
        >
          <e-accumulation-series-collection>
            <e-accumulation-series
              :dataSource="pieChartDataStocks"
              xName="x"
              yName="y"
              startAngle="60"
              :dataLabel="dataLabel"
  ...
  methods: {
    getAllStocksChart() {
      if (
        ...
        for (let stock in this.infoFinances) {
          this.pieChartDataStocks.push({
            x: stock,
            y: ((marketByStock[stock] / totalMarketValue) * 100).toFixed(2),
            text: stock,
          });
        }
    ...
    getSectorChart() {
      ...
      for (let index in this.info) {
        if (sectors.has(this.info[index]["sector"])) {
          sectors.set(
            this.info[index]["sector"],
            sectors.get(this.info[index]["sector"]) + 1
          );
        } else {
      ...
      for (let [key, value] of sectors) {
        this.pieChartData.push({
          x: key,
          y: ((value / Object.values(this.info).length) * 100).toFixed(2),
          text: key + " " + value,
        });
      }
